...
<script lang="ts">
	import { DARK_THEME, LIGHT_THEME } from "$lib/globals/theme";
    import theme from "$lib/stores/theme";

    export const outline: boolean = false;
    export const hideLabel: boolean = false;
    
    const switchTheme = () => {
        try {
            if($theme===DARK_THEME) theme.set(LIGHT_THEME);
            else theme.set(DARK_THEME);
        } catch (error) {
            console.error(error);
        }
    }
